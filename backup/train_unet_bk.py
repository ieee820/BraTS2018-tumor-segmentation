...
from test_unet import validate, AverageMeter
...
def main():
    ...
    for i, (data, target) in enumerate(train_loader, args.start_iter):
        ## validation
        ...
        data, target = [t.cuda(non_blocking=True) for t in (data, target)]

        # compute output
        output = model(data) # nx5x9x9x9, target nx9x9x9
